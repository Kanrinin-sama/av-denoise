        ...
        if let Some(ext) = ext.as_deref() {
            if Y4M_EXTENSIONS.contains(&ext) {
                let reader = BufReader::new(
                    File::open(input).map_err(|e| DecoderError::FileReadError { cause: e.to_string() })?,
                );
                let decoder = DecoderImpl::Y4m(y4m::decode(Box::new(reader) as Box<dyn Read>).map_err(
                    |e| match e {
                        y4m::Error::EOF => DecoderError::EndOfFile,
                        _ => DecoderError::GenericDecodeError { cause: e.to_string() },
                    },
                )?);
            ...
            if ext == "vpy" {
                ...
                let decoder = DecoderImpl::Vapoursynth(VapoursynthDecoder::from_file(input, HashMap::new())?);
                let video_details = decoder.video_details()?;
                return Ok(Decoder {
                    decoder,
                    video_details,
...
"#,
            ...
            let decoder = DecoderImpl::Vapoursynth(VapoursynthDecoder::from_script(&script, HashMap::new())?);
            let video_details = decoder.video_details()?;
            return Ok(Decoder {
                decoder,
                video_details,
    ...
    pub fn seek_video_frame(&mut self, frame_index: usize) -> Result<(), DecoderError> {
        if frame_index >= self.video_details.total_frames.unwrap_or(0) {
            return Err(DecoderError::EndOfFile);
        }
        if !matches!(self.decoder, DecoderImpl::Ffms2(_)) {
    ...
    #[inline]
    #[cfg(feature = "vapoursynth")]
    pub fn get_video_frame<T: Pixel>(&mut self, frame_index: usize) -> Result<Frame<T>, DecoderError> {
...
impl DecoderImpl {
    ...
    pub(crate) fn read_video_frame<T: Pixel>(
        ...
        match self {
            Self::Y4m(dec) => helpers::y4m::read_video_frame::<Box<dyn Read>, T>(dec, cfg, luma_only),
            #[cfg(feature = "vapoursynth")]
