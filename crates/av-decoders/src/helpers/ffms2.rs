...
impl Ffms2Decoder {
    ...
    pub fn set_output_format(
        ...
        unsafe {
            FFMS_SetOutputFormatV2(
                self.video_source,
                // I HATE C
                [video_info_to_pixel_format(bit_depth, chroma_subsampling)?, -1].as_ptr(),
                width as i32,
                height as i32,
    ...
    fn get_index(input: &Path) -> Result<FfmsIndex, DecoderError> {
        ...
        let idx = if idx.is_null() {
            ...
            let idx = unsafe {
                ...
                FFMS_TrackTypeIndexSettings(idxer, FFMS_TrackType::FFMS_TYPE_ATTACHMENT as i32, 0, 0);
    ...
    unsafe fn get_video_details(video: *mut FFMS_VideoSource) -> Result<VideoDetails, DecoderError> {
        ...
        unsafe {
            ...
            let frame_rate = Rational32::new((*props).FPSNumerator as i32, (*props).FPSDenominator as i32);
